  ...
  try{
    ...
    if(parsed && HEROES[parsed.heroId]){
      state = {
        ...
        diceCount: Math.min(
          6,
          Math.max(1, Number(parsed.diceCount) || 5)
        ),

        dice: Array.isArray(parsed.dice)
          ? parsed.dice.slice(0,6)
          : [1,1,1,1,1,1],

        held: Array.isArray(parsed.held)
          ? parsed.held.slice(0,6)
          : [false,false,false,false,false,false],

        rolls: Math.max(
          0,
          Number(parsed.rolls) || 0
        )
      };

      while(state.dice.length < 6){
        state.dice.push(1);
      }

      while(state.held.length < 6){
        state.held.push(false);
      }
    }
...
function render(){
  ...
  for(
    ...
    die.setAttribute(
      "aria-label",
      `Dé ${i+1}, face ${face.value}`
    );

    /*
      Si la face possède une image,
      on affiche l'image.

      Sinon on affiche le système
      classique chiffre + symbole.
    */
    ...
    if(face.image){
      ...
      die.innerHTML = `
        ${
          state.held[i]
            ? '<span class="hold-badge">GARDÉ</span>'
            : ''
        }
...
function changeDiceCount(delta){
  ...
  const next =
    Math.min(
      ...
      Math.max(
        1,
        state.diceCount + delta
      )
    );

  if(
    next === state.diceCount
  ){
    return;
  }
...
$("#minusDice").addEventListener(
  "click",
  () =>
    changeDiceCount(-1)
);

$("#plusDice").addEventListener(
  "click",
  () =>
    changeDiceCount(1)
);
